...
async function loadSettings() {
  try {
    ...
    // Populate profile - Load actual display name from user endpoint
    ...
    // Populate notifications - FIXED: Changed notifyPingSpihe to notifyPingSpike
    const notif = data.notifications || {};
    document.getElementById("notifyPingSpike").checked = notif.notify_on_ping_spike !== false;
...
async function saveSettings() {
  try {
    ...
    const settings = {
      thresholds,
      notifications: {
        // FIXED: Changed notifyPingSpihe to notifyPingSpike
        notify_on_ping_spike: document.getElementById("notifyPingSpike").checked,
...
async function saveProfile() {
  try {
    ...
    if (data.success) {
      ...
      // Reload page after 1.5 seconds to show updated display name
      setTimeout(() => {
