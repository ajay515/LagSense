...
async function loadUsername() {
  ...
  // Try to load actual display name from backend
  try {
...
async function loadSessions() {
  ...
  try {
    ...
    // FIXED: Backend now returns objects with id and start_time
    sessions.forEach(s => {
      const opt = document.createElement("option");
      opt.value = s.id;  // Use session ID instead of timestamp
      opt.textContent = new Date(s.start_time).toLocaleString();
      select.appendChild(opt);
    });
  } catch (err) {
