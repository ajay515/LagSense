...
# FIXED: Return session objects with ID instead of just timestamps
...
# FIXED: Changed session_id from str to int
@app.get("/session/{user_id}/{game}/{session_id}")
def analyze_session(user_id: int, game: str, session_id: int, db: Session = Depends(get_db)):
    try:
        db_session = db.query(DBSession).filter(
...
def get_statistics(user_id: int, db: Session = Depends(get_db)):
    try:
        ...
        # FIXED: Better handling of best/worst game
        if games and len(games) > 0:
            best_game = min(games.items(), key=lambda x: statistics.mean(x[1]))[0]
            worst_game = max(games.items(), key=lambda x: statistics.mean(x[1]))[0] if len(games) > 1 else best_game
        else:
            best_game = "N/A"
            worst_game = "N/A"
